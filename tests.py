...
class MyTest(unittest.TestCase):
  ...
  def testSoldier(self):
    ...
    self.assertEqual(callable(soldier.attack), True)
    self.assertEqual(len(signature(soldier.attack).parameters), 0)
    self.assertEqual(soldier.attack(), strength)
    self.assertEqual(callable(soldier.receiveDamage), True)
    self.assertEqual(len(signature(soldier.receiveDamage).parameters), 1)
  ...
  def testSaxon(self):
    ...
    self.assertEqual(saxon.receiveDamage(45), 'A Saxon has received 45 points of damage')
    self.assertEqual(saxon.receiveDamage(10), 'A Saxon has received 10 points of damage')
    self.assertEqual(saxon.receiveDamage(health),'A Saxon has died in combat')
  ...
  def testWar(self):
    ...
    war.addViking(viking)
    war.addSaxon(saxon)
    self.assertEqual(war.saxonAttack(), viking.name + ' has received '+ str(saxon.strength) +' points of damage')

    self.assertEqual(callable(war.showStatus), True)
    self.assertEqual(len(signature(war.showStatus).parameters), 0)
    for i in range(2):
      war.vikingAttack()
    self.assertEqual(war.showStatus(), 'Vikings have won the war of the century!')
    ...
    for i in range(11):
      war.saxonAttack()
    self.assertEqual(war.showStatus(), 'Saxons have fought for their lives and survive another day...')
    war.addViking(viking)
    self.assertEqual(war.showStatus(), 'Vikings and Saxons are still in the thick of battle.')
